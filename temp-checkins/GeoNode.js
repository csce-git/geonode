...
GeoNode.SearchTable = Ext.extend(Ext.util.Observable, {
    ...
    updateControls: function () {
        ...
        this.pagerLabel.setText(this.showingText + ' ' + minItem + '-' + maxItem + ' ' + this.ofText + ' ' + total);
    },
...
GeoNode.UserSelector = Ext.extend(Ext.util.Observable, {
    ...
    doLayout: function () {
        ...
        function addSelectedUser() {
            ...
            if (index != -1 && this.selectedUsers.store.findExact('username', value) == -1) {
                ...
                listeners: {
                    load: function (store, recs, opts) {
                        ...
                        store.filterBy(function (rec) {
                            return this.editors.findExact("username", rec.get("username")) == -1 && this.managers.findExact("username", rec.get("username")) == -1;
                        }, this);
                    },
                    scope: this
                }
...
GeoNode.BoundingBoxWidget = Ext.extend(Ext.util.Observable, {
    ...
    applyConstraint: function (query) {
        if (this.hasConstraint()) {
            ...
            bounds.transform(new OpenLayers.Projection(this.viewerConfig.map.projection), new OpenLayers.Projection("EPSG:4326"));
    ...
    initFromQuery: function (grid, query) {
        if (query.bbox) {
            ...
            if (bounds) {
                bounds.transform(new OpenLayers.Projection("EPSG:4326"), new OpenLayers.Projection(this.viewerConfig.map.projection));
...
GeoNode.DataCart = Ext.extend(Ext.util.Observable, {
    ...
    doLayout: function () {
        var widgetHTML = '<div class="selection-table"></div>' + '<div class="selection-controls"></div>' + '<div class="selection-ops></div>"';
        var el = Ext.get(this.renderTo);
        el.update(widgetHTML);
